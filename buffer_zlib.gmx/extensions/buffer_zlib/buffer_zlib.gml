...
/// buffer_deflate(buffer, offset, size, compression_level[1..9], ?out_buffer): Compresses a part of a buffer to a new or given buffer.
...
if (!buffer_deflate_raw1(buffer_zlib_get_address(l_source), l_offset, l_total, argument[3])) {
    return -1;
}
...
var l_avail, l_address = buffer_zlib_get_address(l_result);
if (l_total > 0) do {
    ...
    if (l_avail < 0) {
        ...
        l_address = buffer_zlib_get_address(l_result);
    }
} until (l_avail > 0);
...
/// buffer_inflate(buffer, offset, size, ?out_buffer): Decompresses a part of a buffer to a new or given buffer.
...
if (!buffer_inflate_raw1(buffer_zlib_get_address(l_source), l_offset, l_total)) {
    return -1;
}
...
#define buffer_zlib_get_address
/// buffer_zlib_get_address(buffer)

if (os_type = os_android) {
    return string(buffer_get_address(argument0));
} else {
